...
window.Modules.dashboard = (() => {
  ...
  function renderYearlyActivity() {
    ...
    // >>> Correção: contar ENTRADA no status por histórico (1x por processo/ano),
    // com fallback por status_since quando não há histórico.
    ...
    // 1) Histórico (cachedStatusHistory)
    ...
    // 2) Fallback (status_since do processo no ano)
    (cachedProcesses || []).forEach(proc => {
